      ...
      <form action="" class="form_field" @submit.prevent="handleSubmit(send(userAccount))">
        <div class="newsletter">
          <div class="phone_label">
            ...
            <span class="counttimer" v-if="getPhoneCode">{{ count }}秒後重發簡訊啟用碼</span>
            <span class="resend" @click="resendLetter(userAccount)" v-else>重發簡訊啟用碼</span>
          </div>
...
export default {
  ...
  methods: {
    send: function (phone) {
      ...
      vm.$store.dispatch('phoneConfirm', sendData)
    },
    resendLetter: function (phone) {
      const vm = this
      vm.$store.dispatch('resendLetter', phone).then(function () {
        vm.countDownTimer()
      })
    },
    countDownTimer () {
  ...
  destroyed () {
    const vm = this
    document.cookie = 'pResendActive=;expires=' + (new Date(0)).toGMTString()
    vm.$store.commit('CLEARDATA')
  }
}
</script>
<style scoped>
